...
Drop a candidate if:
  ...
  2. Rooftop / top rail: drop when no window in the photo has its center
     higher than the balcony center (image Y up = smaller cy).
"""
...
def has_window_center_above(
    ...
    *,
    ...
    """True if some window center is higher than the balcony center.

    Compare every window in the facade photo (not X-scoped). Image Y grows
    downward, so a higher window center has a smaller cy than the balcony.
    """
    if not window_boxes:
        return False
...
def no_window_above(
    ...
    *,
    ...
    """True when no window center in the photo lies above the balcony center."""
    return not has_window_center_above(
        box, window_boxes, ih=ih, margin_frac=margin_frac
    )
    ...
    *,
    ...
    for box in boxes:
        ...
        rooftop = no_window_above(
            b, win_boxes, ih=ih, margin_frac=above_margin_frac
        )
        reasons: list[str] = []
        if below:
        ...
        keep = (not below) and (not rooftop)
